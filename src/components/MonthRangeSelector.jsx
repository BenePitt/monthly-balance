...
export default function MonthRangeSelector() {
  ...
  function handleStartChange(e) {
    ...
    if (newStart > currentEnd) {
      dispatch({
        type: 'SET_DATE_RANGE',
        payload: { startYear: year, startMonth: month, endYear: year, endMonth: month },
      });
    } else {
  ...
  function setPreset(preset) {
    ...
    dispatch({
      type: 'SET_DATE_RANGE',
      payload: { startYear: sY, startMonth: sM, endYear: eY, endMonth: eM },
    });
  }
  ...
  return (
    <div className="month-range-selector">
      ...
      <div className="month-range-presets">
        <button className="btn btn-sm btn-outline" onClick={() => setPreset('current')}>
          Aktueller Monat
        </button>
        <button className="btn btn-sm btn-outline" onClick={() => setPreset('last')}>
          Letzter Monat
        </button>
        <button className="btn btn-sm btn-outline" onClick={() => setPreset('3months')}>
          Letzte 3 Monate
        </button>
        <button className="btn btn-sm btn-outline" onClick={() => setPreset('6months')}>
          Letzte 6 Monate
        </button>
        <button className="btn btn-sm btn-outline" onClick={() => setPreset('year')}>
          Aktuelles Jahr
        </button>
        <button
