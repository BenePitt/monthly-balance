...
export function sanitizeTransaction(raw) {
  ...
  return {
    ...
    amount: raw.amount,
    purpose: raw.purpose,
    category: typeof raw.category === 'string' && raw.category.trim() ? raw.category.trim() : 'Sonstiges',
    partner: typeof raw.partner === 'string' && raw.partner.trim() ? raw.partner.trim() : 'Unbekannt',
...
export function validateTransaction(raw) {
  ...
  if (typeof raw.date !== 'string' || !/^\d{4}-\d{2}-\d{2}$/.test(raw.date)) errors.push('Ungültiges Datum');
  if (raw.type !== 'income' && raw.type !== 'expense') errors.push('Ungültiger Typ');
  if (typeof raw.amount !== 'number' || !isFinite(raw.amount) || raw.amount <= 0) errors.push('Ungültiger Betrag');
...
export function sanitizeBalanceSettings(raw) {
  ...
  return {
    startBalance: typeof raw.startBalance === 'number' && isFinite(raw.startBalance) ? raw.startBalance : 0,
    currentBalance: typeof raw.currentBalance === 'number' && isFinite(raw.currentBalance) ? raw.currentBalance : 0,
...
export function parseImportPayload(parsed) {
  ...
  if (!parsed || typeof parsed !== 'object') {
    return { version: 0, transactions: [], balanceSettings: null, warnings: ['Ungültiges JSON-Format'] };
  }

  const version = typeof parsed.version === 'number' ? parsed.version : 0;

  if (version > CURRENT_VERSION) {
    warnings.push(`Datei wurde mit einer neueren Version exportiert (v${version}). Import wird trotzdem versucht.`);
  }
